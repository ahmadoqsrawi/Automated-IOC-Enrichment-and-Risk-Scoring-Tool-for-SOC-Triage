...
class TestGenerateSummary:
    def _make(self, **kwargs) -> EnrichmentResult:
        r = EnrichmentResult(ioc="1.2.3.4", ioc_type="ip")
        for k, v in kwargs.items():
            setattr(r, k, v)
        return r

    def test_clean_ioc_returns_no_indicators(self):
        r = self._make(risk_score=5, verdict="low", vt_malicious=0, abuse_score=0, urlhaus_status="not_found")
        summary = generate_summary(r)
        assert "no strong malicious" in summary.lower()

    def test_high_vt_mentioned_in_summary(self):
        r = self._make(risk_score=65, verdict="high", vt_malicious=8, abuse_score=0, urlhaus_status="not_found")
        summary = generate_summary(r)
        assert "virustotal" in summary.lower()
        assert "8" in summary

    def test_high_abuse_mentioned_in_summary(self):
        r = self._make(risk_score=75, verdict="high", vt_malicious=0, abuse_score=92, urlhaus_status="not_found")
        summary = generate_summary(r)
        assert "abuseipdb" in summary.lower()
        assert "92" in summary

    def test_urlhaus_online_mentioned(self):
        r = self._make(risk_score=80, verdict="high", vt_malicious=0, abuse_score=0, urlhaus_status="online")
        summary = generate_summary(r)
